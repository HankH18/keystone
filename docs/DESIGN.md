...
splink deterministic mode generates candidate pairs from match keys (blocking on the three key classes); a deterministic rule cascade decides link/no-link; survivorship per field = documented precedence (app DB > CRM > payments for identity fields; payments authoritative for money fields; **in-source tiebreak = the lexicographically smallest source ref** — for CRM contacts, the lowest `crm_id`; contract §4.6). Fuzzy similarity scores feed **evidence signals only**, never link decisions. Output, per contract §4.7 — **both** tables:
- `entity_links(canonical_id, source_ref, resolved_ref, link_class, method, generation)` — accepted links only; `method` is the id of the first cascade rule that fired (`L1|L2|L3|P1|P2|P3|E1|E2|D2`) and is read directly by `R-004`.
- `entity_link_candidates(source_ref, key_class, resolved_ref, generation, decision, reason)` — **every** match-key resolution, accepted or not; `R-010` is evaluated over this table, never over `entity_links`.
...
- `field_lineage(person_key, field_path, generation, value_canon, source_ref)` — keyed on `person_key` (contract §4.1, §3); `field_path` is a source-qualified path from the `COMPARED_FIELDS` vocabulary and `value_canon` is `canon_value(v)`. Index `(person_key, field_path, generation)`; oscillation = window scan for value A,B,A.
- `source_generations(source_id, generation, entity_type, expected_count, loaded_count, complete bool)` — per-run completeness ledger; drives the `degraded` run marking and the absence-rule skip (contract §5.3).
