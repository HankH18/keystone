...
-- @scope: stg_student
--
-- SS5.5 C6 -- field disagreement.
--
-- "linked person with >=1 disagreeing `COMPARED_FIELDS` comparison whose
--  disagreeing-path set is **not** wholly a subset of `SENSITIVE_FIELDS`"
-- entity_refs: identity refs.
--
-- The partition against R-014 is SS2.4's, evaluated over the *set* of disagreeing
-- rows: if every disagreeing row is wholly sensitive the conflict is C14 and this
-- rule must not also emit C6 (SS5.7 rule 1). A mixed set emits C6 only, with the
-- sensitive paths still listed in `disagreeing_fields`. Because the two rules
-- partition the same computation, `PRECEDENCE` rule 1 has nothing left to do -- it
-- is retained in the runner as the total function SS5.7 pins it to be.
--
-- FP guard (`G17`, `G18`, `G21`, `G23`, `G37`): dirt normalizes away, a NULL operand
-- is `unchecked`, every household is funnel-uniform, and the C3/C10 contacts carry
-- `grade`/`lifecycle_stage` identical to the student they link to, so SS4.6
-- survivorship cannot manufacture a disagreement.
...
-- `survived_enrollment_ref`) instead of being restated here and in R-014.
--
...
decided AS (
    SELECT sides.source_ref,
           ...
           (
            ...
            AND NOT rolled.wholly_sensitive
           ) AS fires
      FROM sides
