...
`useReactTable`), TanStack Query, a ~100-line History-API router, vitest +
Testing Library, ESLint, Playwright + axe-core.

## Routes

| Route | What it shows |
| --- | --- |
| `/` | Overview — reconciles each of the fourteen per-type conflict counts against `/api/scorecard`, and says **Mismatch** in words when they disagree |
| `/conflicts` | Conflict list; server-side filter by source / type / status, server-side pagination |
| `/conflicts/:id` | Row detail: disagreeing sources, disagreeing fields as source-qualified paths grouped into their COMPARED_FIELDS rows, the evidence packet, confidence, fingerprint, status, and the approve/reject action inline |
...
- **ASSUMED (A1–A10)** — shapes DESIGN does *not* promise and this dashboard had
  ...
  the two `jsonb` interiors the UI reads (`evidence.observed_values`,
  `action.target_path`). Each is confined to `contract.ts` +
  ...
  the guard that makes it visible. **Carry that array into the service tickets.**
...
and no type column** — serving those two requires a JOIN to `conflicts` that the
service may never implement. A service that does not know a query param does not
usually reject it: it answers `200` with the **unfiltered** page. On a reviewer
surface that is worse than an error, and no proposal field can contradict it, so
the client cannot verify it the way it verifies the rest.
...
**The service API does not exist yet** (T-5/T-7/T-8 build it). The real HTTP
client is the default; the mock in `src/mocks/` is reached only with
`VITE_USE_MOCK_API=1`, is seeded from the committed `golden/` artifacts, and
announces itself with a banner. Read `src/mocks/README.md` — it lists exactly
what is real, what is invented, and what has to change when the API lands.
...
- **Contrast is computed, not eyeballed.** `e2e/contrast.spec.ts` walks every
  ...
  background, and checks the WCAG ratio against 4.5:1 (3:1 for large text).
...
- **axe-core on every route**, including a detail view, at **zero violations of
